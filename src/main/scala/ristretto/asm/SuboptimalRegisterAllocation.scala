...
object SuboptimalRegisterAllocation {
  ...
  // We use R14..R15 and XMM14..XMM15 as our temporary registers.
  ...
  def regalloc(map: OffsetMap, insn: Insn): List[Insn] = insn match {
    ...
    case AddF(src, dst) => loadf(map, src, XMM14()) ++ loadf(map, dst, XMM15()) ++ ( AddF(XMM14(), XMM15()) :: storef(map, XMM15(), dst) )
    case SubF(src, dst) => loadf(map, src, XMM14()) ++ loadf(map, dst, XMM15()) ++ ( SubF(XMM14(), XMM15()) :: storef(map, XMM15(), dst) )
    case MulF(src, dst) => loadf(map, src, XMM14()) ++ loadf(map, dst, XMM15()) ++ ( MulF(XMM14(), XMM15()) :: storef(map, XMM15(), dst) )
    case DivF(src, dst) => loadf(map, src, XMM14()) ++ loadf(map, dst, XMM15()) ++ ( DivF(XMM14(), XMM15()) :: storef(map, XMM15(), dst) )
    case MovF(src, dst) => loadf(map, src, XMM14()) ++ storef(map, XMM14(), dst)
    case CmpF(r1, r2) => loadf(map, r1, XMM14()) ++ loadf(map, r2, XMM15()) :+ CmpF(XMM14(), XMM15())
    case I2F(src, dst) => load(map, src, R14()) ++ (I2F(R14(), XMM14()) :: storef(map, XMM14(), dst))
